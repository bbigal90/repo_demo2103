...
//오라클에서 조회된 결과를 담기는 부분이 바로 여기다.........					
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj == jbtn_search || obj == jtf_search) {
			// refreshData(jcb_zdo.getSelectedItem().toString(), jtf_search.getText());
			// refreshData(jcb_zdo.getSelectedItem().toString(),
			// jcb_dong.getSelectedItem().toString());
			refreshData(zdo, jtf_search.getText());
			refreshData(zdo, jcb_dong.getSelectedItem().toString());
		}
	}
		...
		if (e.getClickCount() == 2) {
			...
			for (int i = 0; i < dtm_zipcode.getRowCount(); i++) {
				if (jtb_zipcode.isRowSelected(i)) {
					Integer zipcode = Integer.parseInt((String) dtm_zipcode.getValueAt(i, 1));
				}
			}
		}
	}
